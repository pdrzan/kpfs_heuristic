...
int rf(SCIP* scip, SCIP_SOL** sol, SCIP_HEUR* heur)
{
  ...
  frac = 1;
  for(parte=0;parte<K && frac && nFixed < I->n;parte++){ // itera para cada parte da particao, mas pode parar antes se a solucao ja eh inteira.
#ifdef DEBUG
    printf("\n=====parte=%d K=%d\n", parte, K);
#endif
    // carga do lp
    // load problem into scip
    if(!loadProblem(subscip, "rf", I, 1, fixed)){ // relaxation
